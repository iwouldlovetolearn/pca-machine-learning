...
def distance(x0, x1):
    ...
    distance = np.sqrt((x0 - x1).T @ (x0 - x1)) # <-- EDIT THIS to compute the distance between x0 and x1
    return distance

def angle(x0, x1):
    """Compute the angle between two vectors x0, x1 using the dot product"""
    angle = np.arccos((x0.T @ x1) / np.sqrt(( (x0.T @ x0) * (x1.T @ x1) ))) # <-- EDIT THIS to compute angle between x0 and x1
    return angle
...
def most_similar_image():
    ...
    """
    list1_copy = distances.copy()
    list1_copy.sort()
    a = list1_copy[0]
    list1_copy = [x for x in list1_copy if x != a]
    b = list1_copy[0]
...
def pairwise_distance_matrix(X, Y):
    ...
    for n in range(N):
        for m in range(M): 
            distance1 = np.sqrt((X[n,] - Y[m,]) @ (X[n,] - Y[m,]).T)
            distance_matrix[n,m] += distance1
...
def KNN(k, X, y, x):
    ...
    dist = pairwise_distance_matrix(X, x).flatten() # <-- EDIT THIS to compute the pairwise distance matrix
    ...
    for c in np.unique(classes):
        ypred[c] = len(classes[classes == c]) / k  # <-- EDIT THIS to compute the correct prediction
